...
// Analytics remains admin-only but returns no synthetic production KPIs until real telemetry is connected.
app.get('/api/analytics/summary',(_req,res)=>res.json({timestamp:new Date().toISOString(),source:'NO_REAL_TELEMETRY',modelled:true,infrastructure:{firestoreReads:0,firestoreWrites:0,storageBandwidthMb:0,geminiTokensUsed:0,estimatedMonthlyCostUsd:0,estimatedMonthlyCostArs:0},businessKpis:{totalUsers:0,activeClubs:0,activeAthletes:0,proSubscribers:0,monthlyRevenueArs:0,freeToProConversionRate:'0%',topSearchedSports:[],topProvinces:[]},message:'No hay telemetría analítica real conectada; no se muestran métricas sintéticas.'}));
