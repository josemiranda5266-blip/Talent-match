...
function addFinancialAuditLog(action:string,reason:string,estimatedSavingsUsd:number){financialAuditLogs.unshift({id:`flog_${Date.now()}_${Math.random().toString(36).substring(2,6)}`,timestamp:new Date().toISOString(),action,estimatedSavingsUsd,protectionLevel:getProtectionStatus().status,reason});if(financialAuditLogs.length>100)financialAuditLogs.pop();}
